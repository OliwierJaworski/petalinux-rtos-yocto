...
vdma_setup(vdma_handle* handle){
    TRACE(LOG_TRACE, LOG_ORIGIN("begin vdma_setup"), NULL);
    ...
    if( handle->vdmafp == -1 )
        TRACE(LOG_ERROR, LOG_ORIGIN("open"), NULL);
    ...
    if( (void*) handle->vdma_crp == MAP_FAILED )
        TRACE(LOG_ERROR, LOG_ORIGIN("mmap1"), NULL);
    ...
    if( (void*) handle->bufferHandle->VirtualAddr == MAP_FAILED )
        TRACE(LOG_ERROR, LOG_ORIGIN("mmap2"), NULL);
    ...
    TRACE(LOG_DEBUG, LOG_ORIGIN("checking phys | virt addr assignment"), DEBUG_phys_virt_bprint,
        (unsigned long)handle->bufferHandle);
    
    //if volatile is not used gcc will distille the while loop into := if(tmp & 0x4) -> infinite while loop
    volatile u_int32_t* vcrp = ((u_int32_t*)handle->vdma_crp);
    vcrp[OFFSET_VDMA_MM2S_CONTROL_REGISTER/4] = VDMA_CONTROL_REGISTER_RESET; //reset 
    while(vcrp[OFFSET_VDMA_MM2S_CONTROL_REGISTER/4] & 0x4);
    ...
    vcrp[OFFSET_VDMA_MM2S_HSIZE /4] = width * depth;
    vcrp[OFFSET_VDMA_MM2S_VSIZE /4] = height; 

    if(vcrp[OFFSET_VDMA_MM2S_CONTROL_REGISTER/4] & VDMA_CONTROL_STATE_STOP)
        TRACE(LOG_ERROR, LOG_ORIGIN("control register at offset 0 has status IDLE"), NULL);
    TRACE(LOG_TRACE, LOG_ORIGIN("end vdma_setup"), NULL);
}

int 
vdma_remove(vdma_handle* handle){
    TRACE(LOG_TRACE, LOG_ORIGIN("begin vdma_remove"), NULL);
    munmap((void*)handle->vdma_crp, DEV_VDMA_SIZE);
    munmap((void*)handle->bufferHandle->VirtualAddr, BUFFER_SIZE);
    close(handle->vdmafp);
    close(handle->bufferfp);
    TRACE(LOG_TRACE, LOG_ORIGIN("end vdma_remove"), NULL);
}

int 
vdma_regcheck(vdma_handle* handle){
    TRACE(LOG_TRACE, LOG_ORIGIN("begin vdma_regcheck"), NULL);
